...
func (pq *PermissionQuery) QueryUserPermissions() *UserPermissionQuery {
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withUserPermissions: pq.withUserPermissions.Clone(),
...
func (pq *PermissionQuery) WithUserPermissions(opts ...func(*UserPermissionQuery)) *PermissionQuery {
	query := (&UserPermissionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserPermissions = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withUserPermissions; query != nil {
		if err := pq.loadUserPermissions(ctx, query, nodes,
...
func (pq *PermissionQuery) loadUserPermissions(ctx context.Context, query *UserPermissionQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *UserPermission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Permission)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
